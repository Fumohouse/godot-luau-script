...
TEST_CASE("luau script: instance") {
	...
	SECTION("instantiation") {
		REQUIRE(script->load_table(LuauRuntime::VM_CORE) == OK);
		lua_getref(T, script->get_table_ref(LuauRuntime::VM_CORE));
...
TEST_CASE("luau script: inheritance") {
	...
	SECTION("IsScript") {
		...
		SECTION("base") {
			REQUIRE(base->load_table(LuauRuntime::VM_CORE) == OK);
			lua_getref(T, base->get_table_ref(LuauRuntime::VM_CORE));
