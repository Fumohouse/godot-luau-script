...
Error LuauScript::finish_load() {
	...
	lua_getref(L, table_refs[LuauRuntime::VM_SCRIPT_LOAD]);
...
Error LuauScript::load_table(LuauRuntime::VMType p_vm_type, bool p_force) {
#define LOAD_TABLE_METHOD "LuauScript::load_table"
	...
	if (_is_loading) {
		error(LOAD_TABLE_METHOD, "Cyclic dependency detected: Requested script load when it was already loading", 1);
	...
	if (try_load(T) == OK) {
		...
		if (luascript_class_table_get_script(T, 1) != this) {
			error(LOAD_TABLE_METHOD, "Script did not return a valid class table. Did you return a table processed by `gdclass`?", 1);
			lua_pop(L, 1); // thread
			_is_loading = false;
			return ERR_COMPILATION_FAILED;
		}
...
void LuauScript::unref_table(LuauRuntime::VMType p_vm) {
	if (!table_refs[p_vm])
		return;
...
Error LuauScript::reload_tables() {
	for (int i = 0; i < LuauRuntime::VM_MAX; i++) {
		if (i != LuauRuntime::VM_SCRIPT_LOAD && !table_refs[i])
			continue;

		Error err = load_table(LuauRuntime::VMType(i), true);
		if (err != OK)
			return err;
	}
...
void *LuauScript::_instance_create(Object *p_for_object) const {
	LuauRuntime::VMType type = LuauRuntime::VM_USER;
...
LuauScriptInstance::LuauScriptInstance(const Ref<LuauScript> &p_script, Object *p_owner, LuauRuntime::VMType p_vm_type) :
...
#ifdef TESTS_ENABLED
	// Tests are run at this stage (before LuauRuntime and LuauCache are initialized and after _init is called)
	// in order to ensure singletons/methods are all registered and available for immediate retrieval.
	run_tests();
#endif // TESTS_ENABLED

	// Initialize LuauInterface first, deinit last due to LuauRuntime dependency
	interface = memnew(LuauInterface);
	luau = memnew(LuauRuntime);
